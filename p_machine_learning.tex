...
Maximizing log-likelihood of $P(w_t | \mathcal{C}_t^{(c)})$ is equivalent to minimizing the cross entropy:
\begin{align}
E(\mathbf{\hat y}_t, \mathbf{y}_t) &= - \mathbf{y}_t^{\top} log(\mathbf{\hat y}_t) = - log(P(w_t | \mathcal{C}_t^{(c)}))
\end{align}
Because $\mathbf{y}_t$ is a one-hot vector.
...
\begin{tikzpicture}[shorten >=1pt,->,draw=black!50, node distance=\layersep]
    ...
    \node[annot,right of=hl] (ol) {Output layer: $f$ of size $V$};
