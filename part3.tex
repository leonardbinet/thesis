...
\subsubsection*{Previous layer definition}
...
\begin{tikzpicture}[shorten >=1pt,->,draw=black!50, node distance=\layersep]
	...
	\foreach \name / \y in {1,...,3}
        \node[output neuron, pin=right:$h_{\y}$ ] (O-\name) at (\layersep*2,-\y) {};
    ...
    % Annotate the layers
    \node[annot,above of=H-1, node distance=1cm] (hl) {Hidden layer: $l$ of size $d$};
    \node[annot,left of=hl] {Input layer: $x$};
    \node[annot,right of=hl] (ol) {Output layer: $h$ of size $K$};
    \node[annot,right of=ol] {True layer: $y$ of size $K$};

    \node[annot] (W) at (\layersep/2,-5) {$f|W$};
