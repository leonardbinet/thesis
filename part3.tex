...
We now describe the cost function that we’ll use for softmax regression. In the equation below, $\mathds{1}\{\cdot\}$ is the ”‘indicator function,”’ so that $\mathds{1}\{\hbox{a true statement}\}=1$, and $\mathds{1}\{\hbox{a false statement}\}=0$. For example, $\mathds{1}\{2+2=4\}$ evaluates to 1; whereas $\mathds{1}\{1+1=5\}$ evaluates to 0. 
...
\begin{align} 
	H_\theta(x) 
		...
		&= 
		\begin{bmatrix} 
			\frac{1}{1 + \exp(-\theta^{(1)\top} x)} \exp(\theta^{(1)\top} x ) \\ \frac{1}{1 + \exp(-\theta^{(2)\top} x)}\exp(\theta^{(2)\top} x ) \\ \vdots \\ \frac{1}{1 + \exp(-\theta^{(K)\top} x)}\exp(\theta^{(K)\top} x ) \\ 
		\end{bmatrix} 
\end{align}
...
Remembering that in binary case $k \in \{0,1\}$:
\begin{align}
	P(Y=k|x) = (h_\theta(x))^k \times (1 - h_\theta(x))^{(1-k)}
\end{align}
Then: $\forall j \in [1, K]$, $k \in \{0,1\}$:
\begin{align}
	P(Y_j=k|x) = (h_\theta^{(j)}(x))^k \times (1 - h_\theta^{(j)}(x))^{(1-k)}
\end{align}
...
\begin{align}
	...
	&= \prod_{i=1}^m \prod_{j=1}^K (h_\theta^{(j)}(x))^{y_j^{(i)}} \times (1 - h_\theta^{(j)}(x))^{(1-y_j^{(i)})}
\end{align}


Another way to write it is: with $\mathcal{P}_i$ the set of indices among K classes which are positives among sample $i$, and $\mathcal{N}_i$ the negatives set: ($|\mathcal{P}_i| + |\mathcal{N}_i|$ = K)

\begin{align}
	L(\theta) &= \prod_{i=1}^m \prod_{j=1}^K P(Y_j=y_j^{(i)} | x^{(i)}) \\
			  &= \prod_{i=1}^m \left[ \prod_{j \in \mathcal{P}_i } P(Y_j=1 | x^{(i)}) \prod_{l \in \mathcal{N}_i} P(Y_j=0 | x^{(i)}) \right]
\end{align}
